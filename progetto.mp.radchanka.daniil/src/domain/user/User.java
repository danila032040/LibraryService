...
public class User extends Entity<UserId> implements Cloneable<User> {
    public static User createNewUser(
        ...
        User createdUser = new User(id, name, surname, address, phoneNumber, new ArrayList<>());
        createdUser.registerDomainEvent(new UserCreatedDomainEvent(createdUser));
        return createdUser;
    }
    ...
    private User(
            ...
            Address address,
            Optional<String> phoneNumber,
            Collection<BookId> borrowedBookIds) {
        super(id);
    ...
    public void addBorrowedBook(BookId bookId) throws BookWasAlreadyBorrowedByTheUserDomainEvent {
    ...
    public void changeAddress(Address address) {
        ...
        this.registerDomainEvent(new UserChangedAddressDomainEvent(this, previousAddress, currentAddress));
    }
    ...
    public void changePhoneNumber(String phoneNumber) {
        ...
        previousPhoneNumber.ifPresentOrElse(previousPhoneNumberValue -> {
            this
                    .registerDomainEvent(
                            new UserChangedPhoneNumberDomainEvent(this, previousPhoneNumberValue, currentPhoneNumber));
        }, () -> {
            this.registerDomainEvent(new UserSpecifiedPhoneNumberDomainEvent(this, currentPhoneNumber));
        });
    }
    ...
    public User createClone() {
        return new User(
                ...
                address,
                phoneNumber,
                borrowedBookIds.stream().map(BookId::createClone).collect(Collectors.toList()));
    }
    ...
    public Collection<BookId> getBorrowedBookIds() {
        return borrowedBookIds.stream().map(BookId::createClone).collect(Collectors.toList());
    }
    ...
    public void returnBorrowedBook(BookId bookId) throws BookWasNotBorrowedByTheUserDomainEvent {
        Objects.requireNonNull(bookId);
        if (!borrowedBookIds.contains(bookId)) {
