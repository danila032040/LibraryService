...
public class RegisterBookCommandHandler implements RequestHandler<RegisterBookCommand, ErrorOr<BookId>> {
	...
	public RegisterBookCommandHandler(Validator<RegisterBookCommand> validator, BookRepository bookRepository,
	...
	public ErrorOr<BookId> handle(RegisterBookCommand request) {
		try {
			...
			Book bookToRegister = Book.createNewBook(bookRepository.generateNewBookId(), request.getName(),
					request.getGenre(), request.getPublicationYear(), request.getAuthorId().map(AuthorId::new),
