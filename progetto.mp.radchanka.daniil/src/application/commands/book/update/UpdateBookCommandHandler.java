...
public class UpdateBookCommandHandler implements RequestHandler<UpdateBookCommand, ErrorOr<Success>> {
	...
	public UpdateBookCommandHandler(Validator<UpdateBookCommand> validator, BookRepository bookRepository,
	...
	private Boolean isAuthorIdNewerForTheBook(Optional<AuthorId> newAuthorId, Book existingBook) {
		return newAuthorId.map(authorId -> existingBook.getAuthorId().isEmpty()
				|| !Objects.equals(existingBook.getAuthorId().get(), authorId)).orElse(false);
	}

	private Boolean isLibraryIdNewerForTheBook(Optional<LibraryId> newLibraryId, Book existingBook) {
		return newLibraryId.map(authorId -> existingBook.getLibraryId().isEmpty()
				|| !Objects.equals(existingBook.getLibraryId().get(), authorId)).orElse(false);
	}
