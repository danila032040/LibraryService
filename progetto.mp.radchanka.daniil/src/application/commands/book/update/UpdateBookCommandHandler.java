...
public class UpdateBookCommandHandler
		implements
			RequestHandler<UpdateBookCommand, ErrorOr<Success>> {
	...
	public ErrorOr<Success> handle(UpdateBookCommand request) {
		try {
			Optional<AuthorId> newAuthorId = request
					.getAuthorId()
					.map(AuthorId::new);
			Optional<LibraryId> newLibraryId = request
					.getLibraryId()
					.map(LibraryId::new);

			Optional<Book> optionalExistingBook = bookRepository
					.getFirst(
							new BookByIdSpecification(
			...
			if (!hasInformationToUpdate)
				return ErrorOr
						.fromResult(
								Success
										.from(
												"Book already contains provided information. Update not needed"));
	...
	private Boolean isAuthorIdNewerForTheBook(
			Optional<AuthorId> newAuthorId,
			Book existingBook) {
		return newAuthorId
				.map(
	...
	private Boolean isLibraryIdNewerForTheBook(
			Optional<LibraryId> newLibraryId,
			Book existingBook) {
		return newLibraryId
				.map(
