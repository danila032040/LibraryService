...
public class UpdateUserCommandHandler implements RequestHandler<UpdateUserCommand, ErrorOr<Success>> {
	...
	public UpdateUserCommandHandler(
			...
			UserRepository userRepository, DomainEventPublisher domainEventPublisher,
	...
	private Boolean isPhoneNumberNewerForTheUser(Optional<String> newPhoneNumber, User existingUser) {
		return newPhoneNumber.map(phoneNumber -> existingUser.getPhoneNumber().isEmpty()
				|| !Objects.equals(existingUser.getPhoneNumber().get(), phoneNumber)).orElse(false);
	}
