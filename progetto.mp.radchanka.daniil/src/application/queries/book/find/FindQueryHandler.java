...
public class FindQueryHandler
	...
	public FindQueryHandler(Validator<FindQuery> validator,
	...
	public ErrorOr<Collection<Book>> handle(FindQuery request) {
		try {
			...
			Specification<Book> specification = buildBookSpecificationFromRequest(
					request);
			SortCriteria<Book> sortCriteria = buildBookSortCriteriaFromRequest(
					request);
	...
	private Specification<Book> buildBookSpecificationFromRequest(
			FindQuery request) {
				...
				.map(
						genre -> SpecificationUtils
								.<Book, String>generateFieldSpecification(
										genre,
										Book::getGenre,
										(searchGenre, bookGenre) -> bookGenre
												.contains(searchGenre)))
				.map(specification::and)
				.orElse(specification);
		specification = request
				.getAuthorId()
	...
	private SortCriteria<Book> buildBookSortCriteriaFromRequest(
			FindQuery request) {
								...
								.map(
										thenSortType -> Pair
												.of(
														...
														thenSortType)));
		thenSortPair
				.ifPresent(x -> builder.thenSortBy(x.getLeft(), x.getRight()));
