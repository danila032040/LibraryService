...
public class MessageFormatSupportingTimePackage extends Format {
    ...
    private void adjustTimeZoneIfArgumentContainsTimeZone(SimpleDateFormat simpleDateTimeFormat, Object argument) {
        if (argument instanceof ZonedDateTime) {
            ZonedDateTime zonedDateTime = (ZonedDateTime) argument;
            simpleDateTimeFormat.setTimeZone(TimeZone.getTimeZone(zonedDateTime.getZone()));
        }
        if (argument instanceof OffsetDateTime) {
            OffsetDateTime offsetDateTime = (OffsetDateTime) argument;
            simpleDateTimeFormat.setTimeZone(TimeZone.getTimeZone(ZoneId.of(offsetDateTime.getOffset().getId())));
        }
    }
    ...
    private void adjustTimeZoneOfFormatsUsingArguments(Object... arguments) {
        ...
        IntStream
                ...
                .boxed()
                .map(index -> Pair.of((SimpleDateFormat) formats[index], arguments[index]))
                .forEach(pair -> adjustTimeZoneIfArgumentContainsTimeZone(pair.getLeft(), pair.getRight()));
    }
    
    private Object convertToDateIfItIsFromTimePackage(Object obj) {
        return objectToDateIfItIsFromTimePackageOtherwiseToTheSameObjectConverter.convert(obj);
    }
}
