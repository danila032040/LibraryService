...
public class InMemoryRepository<TEntity extends Entity<TId>, TId> implements Repository<TEntity, TId> {
    ...
    public void add(TEntity entity) throws AlreadyExistsException {
        if (this.storage.stream().anyMatch(x -> Objects.equals(x.getId(), entity.getId()))) {
            throw new AlreadyExistsException(
                    String.format("Entity with id = %s already exists in the storage", entity.getId().toString()));
        }
        addToStorage(entity);
    }
    
    @Override
    public void addRange(Collection<TEntity> entities) throws AlreadyExistsException {
        List<TId> alreadyPresentIds = StreamUtils
                .distinct(Stream.concat(this.storage.stream().map(Entity::getId), entities.stream().map(Entity::getId)))
                .collect(Collectors.toUnmodifiableList());
        
        if (!alreadyPresentIds.isEmpty()) {
            throw new AlreadyExistsException(GenerateAlreadyExistsExceptionMessage(alreadyPresentIds));
        }
    ...
    public void updateRange(Collection<TEntity> entities) {
        removeRangeFromStorage(entities.stream().map(TEntity::getId).collect(Collectors.toUnmodifiableSet()));
        addRangeToStorage(entities);
    }
    ...
    private Stream<TEntity> applyPagination(Stream<TEntity> stream, Pagination pagination) {
        return stream.skip(pagination.getPageIndex() * pagination.getPageSize()).limit(pagination.getPageSize());
    }
    
    private Stream<TEntity> applySortCriteria(Stream<TEntity> stream, SortCriteria<TEntity> sortCriteria) {
        return stream.sorted(sortCriteria.getSortComparator());
    }
    
    private Stream<TEntity> applySpecification(Stream<TEntity> stream, Specification<TEntity> specification) {
    ...
    private String GenerateAlreadyExistsExceptionMessage(Collection<TId> alreadyPresentIds)
            throws AlreadyExistsException {
        StringBuilder exceptionMessage = new StringBuilder("Entities with ids = {");
